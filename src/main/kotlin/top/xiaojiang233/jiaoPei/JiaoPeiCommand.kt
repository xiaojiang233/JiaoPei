...
class JiaoPeiCommand : CommandExecutor, TabCompleter {
    private val subCommandManager = SubCommandManager()
    private val requestMap = mutableMapOf<String, String>()
    private val bdsmManager = BdsmManager(Utils.getPlugin())
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (args.isEmpty()) {
            sender.sendMessage(Utils.prefixedMessage("&c请指定一个子命令！&7可用命令: &e/jiaopei do <玩家>&7, &e/jiaopei accept <玩家>&7, &e/jiaopei bdsm"))
            return false
        }
